...
"""
Modified default refinement strategy used by MarineSTD-GS.

This version is derived from gsplat 1.4.0 and includes lightweight,
method-specific changes to the densification and pruning schedule required by
MarineSTD-GS.

The refinement schedule was developed for MarineSTD-GS with reference to prior
...
class DefaultStrategy(Strategy):
    ...
    """Opacity value used during opacity reset. If negative, fall back to prune_opa * 2.0."""

    cull_alpha_thresh_post: float = -1.0
    """Opacity pruning threshold used after densification stops. If negative, reuse prune_opa."""

    continue_cull_post_densification: bool = False
    """Whether to continue pruning after densification has stopped."""
    ...
    def step_post_backward(
        ...
        # Stop the strategy entirely after densification unless post-densification
        # pruning is explicitly enabled.
        if step >= self.refine_stop_iter and not self.continue_cull_post_densification:
            return

        self._update_state(params, state, info, packed=packed)

        # Standard densification stage: grow / split / prune.
        if (
        ...
        # Optional post-densification pruning stage:
        # continue pruning after growth/splitting has stopped.
        if (
            ...
            self.prune_opa = (
                ...
                if self.cull_alpha_thresh_post > 0
                else self.prune_opa
            )
        ...
        # Opacity reset is only applied during the densification stage.
        if (
            step > self.refine_start_iter
            and step < self.refine_stop_iter
            ...
            reset_alpha_value = (
                self.reset_alpha_value
                if self.reset_alpha_value > 0
                else self.prune_opa * 2.0
            )
            reset_opa(
                params=params,
                optimizers=optimizers,
                state=state,
                value=reset_alpha_value,
            )
