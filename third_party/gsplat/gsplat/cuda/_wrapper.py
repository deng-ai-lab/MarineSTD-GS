...
def isect_tiles(
    ...
    Returns:
        ...
        - **Flatten ids**. The global flatten indices in [C * N] or [nnz] (packed). [n_isects] 注意 这里的 [n_isects] 是一个数值，具体和 isect_ids 的大小保持对应，意思是每一个交点对应的是给定高斯在全空间中的位置 ？ 有点奇怪，按理说就N就可以了啊
    """
    if packed:
        nnz = means2d.size(0)
...
class _RasterizeToPixels(torch.autograd.Function):
    ...
    def backward(
        ...
        if ctx.needs_input_grad[4]:  # !!!!!!!!!!!!!!!!!!!!!!! 之所以这样子写 因为 backgrounds 在前向中是除了 ctx 的、从0开始算的，位置是4的元素
