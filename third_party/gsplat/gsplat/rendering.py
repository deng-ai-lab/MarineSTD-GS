...
def ns_spherical_harmonics(
    means: Tensor,  # [N, 3]
    colors: Tensor,  # [(C,) N, K, 3] 这里必须得是特征的形式
    ...
    assert C==1 # 目前只接受一个相机的情况
    
    # 1. 检验格式
    assert sh_degree is not None  # 当启用这个函数的时候, sh_degree 必须非空
    ...
    masks = torch.ones_like(dirs[..., 0], dtype=torch.bool)  # [C, N]  # 因为预先计算的没法考虑非投影部分的高斯       
    ...
    return colors.squeeze(0) # [C, N, 3] -> [N, 3] because C=1
...
def rasterization(
    ...
    if sh_degree is None:
        ...
        if packed:
            ...
            colors = spherical_harmonics(sh_degree, dirs, shs, masks=masks)  # [C, N, 3]
            # make it apple-to-apple with Inria's CUDA Backend.
        # 注意这里引入的0.5偏置设置，即使对应的colors对应的是feature的渲染，那么也会引入对应的偏置
        colors = torch.clamp_min(colors + 0.5, 0.0)
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        render_alphas = render_alphas[0]  # discard the rest —— because all feature during the rendering share the same alpha or opacity
    else:
